...
static int dm1_set_alt(struct usb_function *f, unsigned int intf, unsigned int alt)
{
	...
	if (status < 0) {
		pr_debug("fail to activate generic ttyGS%d\n",
			 dm1->port_num);
...
static void dm1_disable(struct usb_function *f)
{
	...
	pr_info("usb: %s generic ttyGS%d deactivated\n", __func__,
		dm1->port_num);
	gserial_disconnect(&dm1->port);
}
...
dm1_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	pr_info("usb: [%s] generic ttyGS%d: %s speed IN/%s OUT/%s\n",
		__func__,
		dm1->port_num,
...
struct usb_function_instance *alloc_inst_dm1(bool dm1_config)
{
	...
	ret = gserial_alloc_line(&fi_dm1->port_num);
	if (ret) {
		kfree(fi_dm1);
		return ERR_PTR(ret);
	}
