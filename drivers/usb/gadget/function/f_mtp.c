...
struct mtp_dev {
	...
	spinlock_t lock; /* protect mtp device */
...
struct {
	...
	.header = {
		.dwLength = cpu_to_le16(sizeof(mtp_ext_config_desc)),
					    ...
					    bool mtp_config)
{
	 ...
	 */
	if (!fi_mtp->dev) {
		pr_err("Error: Create MTP func before linking PTP func with gadget config\n");
