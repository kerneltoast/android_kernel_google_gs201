				  ...
				  struct samsung_dma_prep *param)
{
	...
	case DMA_SLAVE:
		...
		desc = dmaengine_prep_slave_sg(chan, &sg, 1, param->direction,
					       DMA_PREP_INTERRUPT);
		break;
	case DMA_CYCLIC:
		desc = __pl330_prep_dma_cyclic(chan, param->buf, param->len,
					       param->period, param->direction,
					       true, &param->infiniteloop);
		break;
	default:
...
static inline int samsung_dmadev_getposition(unsigned long ch, dma_addr_t *src,
					     dma_addr_t *dst)
{
