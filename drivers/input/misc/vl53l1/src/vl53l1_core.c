	...
	VL53L1_DEV Dev)
{
	...
	struct VL53L1_zone_private_dyn_cfgs_t *pZ = &(pres->zone_dyn_cfgs);
	...
	uint8_t soft_update)
{
	...
	struct VL53L1_xtalk_config_t *pX = &(pdev->xtalk_cfg);
	struct VL53L1_xtalk_calibration_results_t *pC = &(pdev->xtalk_cal);
	uint32_t *pcpo;
	...
	VL53L1_DEV Dev)
{
	...
	struct VL53L1_range_results_t *pR = &(pres->range_results);
	struct VL53L1_xtalk_config_t *pX = &(pdev->xtalk_cfg);
	...
	struct VL53L1_LLDriverResults_t *pres)
{
	...
	struct VL53L1_smudge_corrector_data_t *pdata;
	...
	VL53L1_DEV Dev)
{
	...
	struct VL53L1_system_results_t *pS = &(pdev->sys_results);
