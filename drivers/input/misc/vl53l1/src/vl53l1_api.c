...
static VL53L1_Error SetTargetData(VL53L1_DEV Dev,
	...
	uint8_t device_status, struct VL53L1_range_data_t *presults_data,
	struct VL53L1_TargetRangeData_t *pRangeData)
{
	VL53L1_Error Status = VL53L1_ERROR_NONE;
	struct VL53L1_LLDriverData_t *pdev =
			VL53L1DevStructGetLLDriverHandle(Dev);
	struct VL53L1_tuning_parm_storage_t *tp =
			&(pdev->tuning_parms);
	uint8_t sequency;
	uint8_t FilteredRangeStatus;
	uint8_t SigmaLimitflag;
	...
	if (ExtendedRangeEnabled &&
		...
		|| ((pdev->PreviousStreamCount) ==
			...
			case VL53L1_DEVICEPRESETMODE_HISTOGRAM_MEDIUM_RANGE:
				...
				} else if (RangeDiff > tp->tp_uwr_med_z_6_min &&
			...
			case VL53L1_DEVICEPRESETMODE_HISTOGRAM_LONG_RANGE:
				...
				} else if (RangeDiff > tp->tp_uwr_lng_z_4_min &&
		...
		uint8_t CalibrationOption)
{
	...
	struct VL53L1_CalibrationData_t caldata;
	struct VL53L1_LLDriverData_t *pLLData;
	int i;
