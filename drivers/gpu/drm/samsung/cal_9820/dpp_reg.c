...
u32 dpp_reg_get_irq_and_clear(u32 id)
{
	...
	if (val & DPP_CONFIG_ERROR) {
		...
		cal_log_err(id, "dpp%d config error occur(0x%x)\n",
				id, cfg_err);
	}
...
u32 idma_reg_get_irq_and_clear(u32 id)
{
	...
	if (val & IDMA_CONFIG_ERROR) {
		...
		cal_log_err(id, "dpp%d idma config error occur(0x%x)\n",
				id, cfg_err);
	}
...
u32 odma_reg_get_irq_and_clear(u32 id)
{
	...
	if (val & ODMA_CONFIG_ERROR) {
		...
		cal_log_err(id, "dpp%d odma config error occur(0x%x)\n",
				id, cfg_err);
	}
...
static const struct dpu_fmt dpu_cal_formats_list[] = {
	{
		...
		.name = "NV16M_P210",
		.fmt = DPU_PIXEL_FORMAT_NV16M_P210,
		...
		.name = "NV61M_P210",
		.fmt = DPU_PIXEL_FORMAT_NV61M_P210,
		...
		.name = "NV16M_S10B",
		.fmt = DPU_PIXEL_FORMAT_NV16M_S10B,
		...
		.name = "NV61M_S10B",
		.fmt = DPU_PIXEL_FORMAT_NV61M_S10B,
...
const struct dpu_fmt *dpu_find_cal_fmt_info(enum dpu_pixel_format fmt)
{
	int i;
