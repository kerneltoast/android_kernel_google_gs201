							...
							loff_t *f_pos)
{
	...
	if (debug_enabled)
	pr_info("st33spi Read: status: %zu\n", status);
	...
	size_t count, loff_t *f_pos)
{
	...
	if (debug_enabled)
	pr_info("st33spi Write: %zu bytes\n", count);
	...
	if (debug_enabled)
	pr_info("st33spi Write: status: %zu\n", status);
...
#ifdef VERBOSE
		dev_dbg(&st33spi->spi->dev, "  xfer len %u %s%s%s%dbits %u usec %uHz\n",
...
static struct spi_ioc_transfer *st33spi_get_ioc_message(
...
static void st33spi_power_on(struct st33spi_data *st33spi)
{
	...
	if (st33spi->power_gpio_mode == POWER_MODE_ST33) {
		...
		gpiod_set_value(st33spi->gpiod_se_reset, 1);
		pr_info("%s : st33 set nReset to High\n", __func__);
		usleep_range(5000, 5500);
		gpiod_set_value(st33spi->gpiod_se_reset, 0);
		pr_info("%s : st33 set nReset to Low\n", __func__);
...
static void st33spi_power_set(struct st33spi_data *st33spi, int val)
{
	...
	if (debug_enabled)
	pr_info("st33spi sehal pwr_req: %d\n", val);
...
{
	return gpiod_get_value(st33spi->gpiod_se_reset);
}
							...
							unsigned long arg)
{
	...
	case SPI_IOC_RD_MAX_SPEED_HZ:
			retval = __put_user(st33spi->speed_hz,
			(__u32 __user *)arg);
			break;
	case ST33SPI_IOC_RD_POWER:
			pr_info("st33spi ST33SPI_IOC_RD_POWER\n");
	...
	case ST33SPI_IOC_WR_POWER:
		retval = __get_user(tmp, (__u32 __user *)arg);
		pr_info("st33spi ST33SPI_IOC_WR_POWER %d\n", retval);
		if (retval == 0) {
			st33spi_power_set(st33spi, tmp ? 1 : 0);
			pr_info("SE_POWER_REQ set: %d\n", tmp);
		}
		break;
	default:
		/* segmented and/or full-duplex I/O request */
		/* Check message and copy into scratch area */
		ioc = st33spi_get_ioc_message(cmd,
	...
	if (debug_enabled)
	pr_info("st33spi ioctl retval %d\n", retval);

	return retval;
}

#ifdef CONFIG_COMPAT
static long st33spi_compat_ioc_message(struct file *filp,
	unsigned int cmd, unsigned long arg)
{
	...
	if (debug_enabled)
	pr_info("st33spi compat_ioctl cmd %d\n", cmd);
	...
	spi_dev_put(spi);
	if (debug_enabled)
	pr_info("st33spi compat_ioctl retval %d\n", retval);
	return retval;
}

static long st33spi_compat_ioctl(struct file *filp,
	unsigned int cmd, unsigned long arg)
{
	if (_IOC_TYPE(cmd) == SPI_IOC_MAGIC &&
			_IOC_NR(cmd) == _IOC_NR(SPI_IOC_MESSAGE(0)) &&
...
static int st33spi_open(struct inode *inode, struct file *filp)
{
	...
	if (status) {
		pr_debug("st33spi: nothing for minor %d\n", iminor(inode));
		goto err_find_dev;
	}
	...
	if (debug_enabled)
	pr_info("st33spi: open - force power on\n");
...
static int st33spi_release(struct inode *inode, struct file *filp)
{
	...
	if (!st33spi->users) {
		...
		if (debug_enabled)
		pr_info("st33spi: release - may allow power off\n");
...
static const struct acpi_device_id st33spi_acpi_ids[] = {
		 ...
		 */
		{"SPT0001", ST33SPI_ACPI_DUMMY},
		{"SPT0002", ST33SPI_ACPI_DUMMY},
		{"SPT0003", ST33SPI_ACPI_DUMMY},
		{},
};
...
static int st33spi_parse_dt(struct device *dev, struct st33spi_data *pdata)
{
	...
	if (np) {
		...
		if (pdata->power_gpio_mode == POWER_MODE_ST33) {
			pdata->gpiod_se_reset = devm_gpiod_get(dev, "esereset",
			GPIOD_OUT_LOW);
			if (IS_ERR(pdata->gpiod_se_reset)) {
				pr_err("%s : Unable to request esereset %d\n",
				__func__, IS_ERR(pdata->gpiod_se_reset));
				return -ENODEV;
			}
		} else {
			pr_err("%s: ST54H mode not supported", __FILE__);
		}
	}
	return r;
}
...
static int st33spi_probe(struct spi_device *spi)
{
	...
	if (minor < N_SPI_MINORS) {
		...
		pr_debug("%s : no minor number available!\n", __FILE__);
		status = -ENODEV;
	}
	if (status == 0) {
	...
	pr_debug("%s : st33spi->speed_hz=%d\n", __FILE__, st33spi->speed_hz);
	//{
	//	/* fixed SPI clock speed: 109200000 */
	//	int period = DIV_ROUND_UP(109200000, st33spi->speed_hz);

	//	st33spi_chip_info.cs_idletime = period;
	//	st33spi_chip_info.cs_holdtime = period;
	//}
...
static int __init st33spi_init(void)
{
	...
	st33spi_major = __register_chrdev(0, 0, N_SPI_MINORS, "spi",
	&st33spi_fops);
