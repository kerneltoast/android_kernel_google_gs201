...
{
	struct s2mpg10_pmic *s2mpg10 = rdev_get_drvdata(rdev);
	int i = rdev_get_id(rdev);

	if (atomic_read(&s2mpg10->need_sync[i]) > 0) {
		s2mpg10->turn_off_on_sync[i] = false;
		return 0;
	}
...
static int s2m_disable(struct regulator_dev *rdev)
{
	...
	if (atomic_read(&s2mpg10->need_sync[i]) > 0) {
		s2mpg10->turn_off_on_sync[i] = true;
		return 0;
	}
...
static int s2mpg10_pmic_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->num_regulators; i++) {
		...
		if (s2m_is_enabled(s2mpg10->rdev[i]))
			atomic_set(&s2mpg10->need_sync[i], 1);
		else
			atomic_set(&s2mpg10->need_sync[i], 0);
...
static void s2mpg10_sync_state(struct s2mpg10_pmic *s2mpg10)
{
	...
	for (i = 0; i < s2mpg10->num_regulators; i++) {
		...
		need_sync = atomic_dec_return(&s2mpg10->need_sync[i]);
		if (need_sync < 0)
			continue;

		if (need_sync == 0) {
			if (s2mpg10->turn_off_on_sync[i])
