...
static void s2mpg10_meter_read_acc_data_reg(struct s2mpg10_meter *s2mpg10,
					    u64 *data);
static void s2mpg10_meter_read_acc_count(struct s2mpg10_meter *s2mpg10,
					 u32 *count);
				   ...
				   u32 *count, u64 *timestamp_capture)
{
	...
	if (count)
		s2mpg10_meter_read_acc_count(s2mpg10, count);
...
static void s2mpg10_meter_set_lpf_mode(struct s2mpg10_meter *s2mpg10,
				       s2mpg1x_meter_mode mode)
{
	switch (mode) {
	case S2MPG1X_METER_POWER:
		s2mpg10_write_reg(s2mpg10->i2c, S2MPG10_METER_CTRL4, 0x00);
		break;
	case S2MPG1X_METER_CURRENT:
		s2mpg10_write_reg(s2mpg10->i2c, S2MPG10_METER_CTRL4, 0xFF);
		break;
	}
}
				       ...
				       s2mpg1x_meter_mode mode)
{
	switch (mode) {
	case S2MPG1X_METER_POWER:
		s2mpg10_write_reg(s2mpg10->i2c, S2MPG10_METER_CTRL3, 0x00);
		break;
	case S2MPG1X_METER_CURRENT:
		s2mpg10_write_reg(s2mpg10->i2c, S2MPG10_METER_CTRL3, 0xFF);
		break;
	}
}
					    ...
					    u64 *data)
{
	...
	u8 reg = S2MPG10_METER_ACC_DATA_CH0_1; /* first acc data register */

	for (i = 0; i < S2MPG1X_METER_CHANNEL_MAX; i++) {
		s2mpg10_bulk_read(s2mpg10->i2c, reg, S2MPG1X_METER_ACC_BUF,
				  buf);
					 ...
					 u32 *count)
{
	...
	s2mpg10_bulk_read(s2mpg10->i2c, S2MPG10_METER_ACC_COUNT_1,
					...
					char *buf)
{
	...
	s2mpg10_meter_load_measurement(s2mpg10, S2MPG1X_METER_CURRENT, acc_data,
				       &acc_count, NULL);
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	s2mpg10_meter_load_measurement(s2mpg10, S2MPG1X_METER_POWER, acc_data,
				       &acc_count, NULL);
