...
static int s2m_enable(struct regulator_dev *rdev)
{
	...
	if (atomic_read(&s2mpg11->need_sync[i]) > 0) {
		s2mpg11->turn_off_on_sync[i] = false;
		return 0;
	}
...
static int s2m_disable(struct regulator_dev *rdev)
{
	...
	if (atomic_read(&s2mpg11->need_sync[i]) > 0) {
		s2mpg11->turn_off_on_sync[i] = true;
		return 0;
	}
...
static int s2mpg11_pmic_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->num_regulators; i++) {
		...
		if (s2m_is_enabled(s2mpg11->rdev[i]))
			atomic_set(&s2mpg11->need_sync[i], 1);
		else
			atomic_set(&s2mpg11->need_sync[i], 0);
...
static void s2mpg11_sync_state(struct s2mpg11_pmic *s2mpg11)
{
	...
	for (i = 0; i < s2mpg11->num_regulators; i++) {
		...
		need_sync = atomic_dec_return(&s2mpg11->need_sync[i]);
		if (need_sync < 0)
			continue;

		if (need_sync == 0) {
			if (s2mpg11->turn_off_on_sync[i])
