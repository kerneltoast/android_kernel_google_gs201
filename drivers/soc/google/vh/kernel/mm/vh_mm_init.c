...
static int vh_mm_init(void)
{
	 ...
	 */
	ret = register_trace_android_vh_try_grab_compound_head(vh_try_grab_compound_head, NULL);
	if (ret)
		return ret;
	ret = register_trace_android_vh___get_user_pages_remote(vh___get_user_pages_remote, NULL);
	if (ret)
		return ret;
	ret = register_trace_android_vh_get_user_pages(vh_android_vh_get_user_pages, NULL);
	if (ret)
		return ret;
	...
	if (ret)
		return ret;
	ret = register_trace_android_vh_pin_user_pages(vh_pin_user_pages, NULL);
	return ret;
}
