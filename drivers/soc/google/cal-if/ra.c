...
unsigned int fin_hz_var = FIN_HZ_26M;
					      ...
					      unsigned long rate_hz)
{
	...
	if (rate_hz) {
		unsigned long min_diff = U64_MAX;
		int min_diff_idx = 0;

		for (i = 0; i < pll_clk->rate_count; i++) {
			if (min_diff > abs(rate_hz - prate_table[i].rate)) {
				min_diff = abs(rate_hz - prate_table[i].rate);
				min_diff_idx = i;
			}
		}

		return &prate_table[min_diff_idx];
	}
...
static unsigned long ra_get_pll(struct cmucal_clk *clk)
{
	...
	if (!ra_is_pll_enabled(clk)) {
		...
		if (pll_con0 & PLL_MUX_SEL)
			...
			return (unsigned long)fin_hz_var;
	}
