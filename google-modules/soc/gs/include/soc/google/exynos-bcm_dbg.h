...
#define BCM_TIMER_PERIOD_MAX                    (1000 * 1000)
#define BCM_TIMER_PERIOD_MIN                    (100)
...
#define BCM_EVT_INVALID				(0xFF)
#define BCM_EVT_EVENT_SHIFT(x)			\
			(((x) >= 4) ? (((x) * 8) - 32) : ((x) * 8))
...
#define BCM_EVT_DUMP_ACC_CONT_SHIFT		(0)

#define BCM_CMD_GET(cmd_data, mask, shift)	\
			((cmd_data & (mask << shift)) >> shift)
