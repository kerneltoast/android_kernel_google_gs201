...
#include "../inc/vl53l1_hist_map.h"
#include "../inc/vl53l1_hist_structs.h"
#include "../inc/vl53l1_nvm_map.h"
#include "../inc/vl53l1_nvm_structs.h"
...
static VL53L1_Error select_offset_per_vcsel(struct VL53L1_LLDriverData_t *pdev,
		int16_t *poffset)
{
...
static void vl53l1_diff_histo_stddev(struct VL53L1_LLDriverData_t *pdev,
	struct VL53L1_histogram_bin_data_t *pdata, uint8_t timing, uint8_t HighIndex,
	...
	struct VL53L1_ll_version_t *pdata)
{
	...
	memcpy(pdata, &(pdev->version), sizeof(struct VL53L1_ll_version_t));
	...
	uint8_t read_p2p_data)
{
	...
	struct VL53L1_zone_objects_t *pobjects;
	...
	VL53L1_DEV Dev)
{
	...
	VL53L1_Error status = VL53L1_ERROR_NONE;
	struct VL53L1_LLDriverData_t *pdev = VL53L1DevStructGetLLDriverHandle(Dev);
	struct VL53L1_hist_post_process_config_t *pHP = &(pdev->histpostprocess);
	struct VL53L1_customer_nvm_managed_t *pN = &(pdev->customer);
	...
	struct VL53L1_calibration_data_t *pcal_data)
{
	...
	VL53L1_Error status = VL53L1_ERROR_NONE;
	struct VL53L1_LLDriverData_t *pdev = VL53L1DevStructGetLLDriverHandle(Dev);
	struct VL53L1_xtalk_config_t *pC = &(pdev->xtalk_cfg);
	struct VL53L1_hist_post_process_config_t *pHP = &(pdev->histpostprocess);
	struct VL53L1_customer_nvm_managed_t *pN = &(pdev->customer);
	...
	if (status == VL53L1_ERROR_NONE) {
		...
		memcpy(
			...
			sizeof(struct VL53L1_per_vcsel_period_offset_cal_data_t));
	...
	struct VL53L1_calibration_data_t *pcal_data)
{
	...
	struct VL53L1_LLDriverData_t *pdev = VL53L1DevStructGetLLDriverHandle(Dev);
	struct VL53L1_xtalk_config_t *pC = &(pdev->xtalk_cfg);
	struct VL53L1_customer_nvm_managed_t *pCN = &(pcal_data->customer);
	...
	struct VL53L1_range_results_t *prange_results)
{
	...
	struct VL53L1_ll_driver_state_t *pstate =
			&(pdev->ll_state);
	struct VL53L1_zone_config_t *pzone_cfg =
			&(pdev->zone_cfg);
	struct VL53L1_zone_hist_info_t *phist_info =
	...
	struct VL53L1_hist_post_process_config_t *pHP = &(pdev->histpostprocess);
	struct VL53L1_xtalk_config_t *pC = &(pdev->xtalk_cfg);
	struct VL53L1_low_power_auto_data_t *pL = &(pdev->low_power_auto_data);
	struct VL53L1_histogram_bin_data_t *pHD = &(pdev->hist_data);
	struct VL53L1_customer_nvm_managed_t *pN = &(pdev->customer);
	struct VL53L1_zone_histograms_t *pZH = &(pres->zone_hists);
	struct VL53L1_xtalk_calibration_results_t *pXCR = &(pdev->xtalk_cal);
	uint8_t tmp8;
	uint8_t zid;
	uint8_t i;
	...
	struct VL53L1_GPIO_interrupt_config_t intconf)
{
	...
	memcpy(pintconf, &(intconf), sizeof(struct VL53L1_GPIO_interrupt_config_t));
	...
	struct VL53L1_tuning_parameters_t *ptun_data)
{
	...
	struct VL53L1_LLDriverData_t *pdev = VL53L1DevStructGetLLDriverHandle(Dev);
	struct VL53L1_hist_post_process_config_t *pHP = &(pdev->histpostprocess);
	struct VL53L1_xtalkextract_config_t *pXC = &(pdev->xtalk_extract_cfg);
