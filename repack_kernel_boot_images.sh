...
extract_ramdisk() {
    local ramdisk="$1"
    local staging_dir="$2"
    local ramdisk_decomp=""

    if ${LZ4} -q -t "$ramdisk" 2>/dev/null; then
        ramdisk_decomp="$LZ4_DECOMP"
    else
        echo "ERROR: invalid ramdisk compression type. Only lz4 and gz are supported." >&2
        exit 1
    fi

    cat "$ramdisk" |
        (cd "$staging_dir"; ${ramdisk_decomp} | cpio --extract)
}

create_ramdisk() {
    local staging_dir="$1"
    local newramdisk="$2"
    local ramdisk_type="$3"
    local tmpramdisk="${TEMP_DIR}/tmp_ramdisk"
    local ramdisk_compress=""

    if [[ "$ramdisk_type" == "lz4" ]]; then
        # The kernel can only read the legacy lz4 format. So we need to use the
        # '-l' flag for the ramdisk.
        ramdisk_compress="$LZ4_COMPRESS -l"
    else
        echo "ERROR: invalid ramdisk compression type. Only lz4 and gz are supported." >&2
        exit 1
    fi
    ...
    (cd "$staging_dir"; find . |
        ...
        ${ramdisk_compress} > ${newramdisk} )
}

create_mixed_ramdisk() {
    # Construct a mixed ramdisk that consists of a device ramdisk + a new
    # kernel initramfs.img. This allows us to update kernel modules without
    # replacing other contents in the device ramdisk.
    local device_ramdisk="$1"
    local kernel_dist_dir="$2"
    local newimage="$3"
    local staging_dir="${TEMP_DIR}"/staging
    local modules_dir="${TEMP_DIR}"/modules
    local ramdisk_compress_type=""

    echo extract device $device_ramdisk to staging directory...
    mkdir -p "$staging_dir"
    extract_ramdisk "$device_ramdisk" "$staging_dir"

    echo remove lib/modules...
    rm -rf "$staging_dir"/lib/modules/*

    echo extract $kernel_dist_dir/initramfs.img...
    mkdir "$modules_dir"
    extract_ramdisk "$kernel_dist_dir"/initramfs.img "$modules_dir"

    echo copy lib/modules to staging...
    cp -r "$modules_dir"/lib/modules/* "$staging_dir"/lib/modules

    # Detect and use the compression type of device_ramdisk for the new ramdisk
    if ${LZ4} -q -t "$device_ramdisk" 2>/dev/null; then
        ramdisk_compress_type="lz4"
    else
        echo "ERROR: invalid ramdisk compression type. Only lz4 and gz are supported." >&2
        exit 1
    fi
    echo create new ramdisk...
    create_ramdisk "$staging_dir" "$newimage" "$ramdisk_compress_type"
}
